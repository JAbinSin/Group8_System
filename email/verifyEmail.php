<?php
    ...
    if(!empty($_POST)) {
        ...
        $url = "localhost/github/Group8_System/email/emailVerify.php?selector=" . $selector . "&validator=" . bin2hex($token);
        ...
        $mail->Subject = "Restauday Email Verification Code";
        $mail->Body = "<p>We recieved an email verification request. The link to verify your email is below if you did not make this request, you can ignore this email</p>";
        $mail->Body .= "<p>Here is your email verification link: </br>";
        $mail->Body .= "<a href='$url'>$url</a></p>";
        $mail->AddAddress("$userEmail");
        $mail->Send();

        header("Location: ../php/profile.php?verify=success");
